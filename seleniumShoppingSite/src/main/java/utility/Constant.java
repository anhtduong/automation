package utility;

public class Constant {
    public static final String driverPath = "C:\\driver\\geckodriver-v0.20.1-win64\\";
    //public static final String dataFilePath = "C:\\Anh Duong\\Personal\\Testing\\Selenium\\automation\\maven\\src\\main\\java\\testData\\";
    public static final String dataFilePath = ".\\src\\main\\java\\testData\\";
    public static final String dataFileName = "TestData.xlsx";
    public static final String homePageTitle = "My Store";
    public static final String signInPageTitle = "Login - My Store";
    public static final String myAccountPageTitle = "My account - My Store";
    public static final String myAccountPageInfo = "Welcome to your account. Here you can manage all of your personal information and orders.";
    public static final String createAccountPageTitle = "Login - My Store";
    public enum testOutcome{PASS, FAIL}
    public static final String sheetSignInValidData = "SignInValidData";
    public static final String sheetSignInNotValidData = "SignInNotValidData";
    public enum browserTypes {CHROME, FIREFOX}

}
